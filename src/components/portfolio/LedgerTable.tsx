...
export function LedgerTable({ entries }: { entries: LedgerEntry[] }) {
  if (entries.length === 0) {
    return (
      <p className="num text-sm text-text-muted">&gt; ledger empty_</p>
    );
  }
  ...
  return (
    <ul className="flex flex-col gap-px border border-hairline bg-hairline">
      {entries.map((e) => (
        <li
          key={e.id}
          className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-page px-4 py-3 sm:px-5"
        >
          <div className="min-w-0 flex-1">
            ...
            {e.marketId && e.marketTitle ? (
              <Link
                ...
                className="mt-0.5 block truncate text-sm text-text-muted hover:text-red-bright focus-visible:outline-red"
              >
                {e.marketTitle}
              </Link>
            ) : null}
          ...
          <div className="text-right">
            <p
              className={`num text-sm font-semibold ${
                e.amount > 0 ? "text-red-bright" : "text-text"
              }`}
            >
              {e.amount > 0 ? "+" : ""}
              {formatHC(e.amount)}
